...
mvpa_dir = '/media/tw260/Samsung_T5/fMRI/results/mvpa_analysis/crosstask';

spm('Defaults','FMRI');

analysisdir = strcat(mvpa_dir,'/SecondLevel/Searchlight_crosstask');
...
try
    ...
    for i = 1:numel(subdirs)
        map_unsmoothed = fullfile(mvpa_dir,subdirs{i},'Searchlight_crosstask','res_accuracy_minus_chance.nii');
        spm_smooth(map_unsmoothed,sprintf('%s/%s/Searchlight_crosstask/s10FWHM_res_accuracy_minus_chance.nii',mvpa_dir,subdirs{i}),[10,10,10]);
        
        S.imgfiles{1}{j} = fullfile(mvpa_dir,subdirs{i},'Searchlight_crosstask','s10FWHM_res_accuracy_minus_chance.nii');
        j = j + 1;
    end
