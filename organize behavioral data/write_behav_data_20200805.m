...
for sub = 1:numel(good_sub_name)
    for run = 1:6
        if run <=4
            ...
            for trial = 1:size(data_orig,1)
                ...
                if isequal(acc,'incorrect')
                    ...
                    trialtype{trial} = strcat(data_orig.blockType{trial},'-',data_orig.trialType{trial});
                end
        ...
        elseif run >4
            ...
            for trial = subjtask_ind
                ...
                elseif isequal(data_orig.blockType{trial},'easy') || isequal(data_orig.blockType{trial},'hard')
                    if contains(data_orig.sbjResp{trial},'Old')
                        acc = 'correct';
                        trialtype{trial} = strcat(data_orig.blockType{trial},'-',data_orig.trialType{trial},'-',acc);
            ...
            all_cond_names = {'easy-con-correct','easy-inc-correct','hard-con-correct','hard-inc-correct',...
                'easy-con-incorrect','easy-inc-incorrect','hard-con-incorrect','hard-inc-incorrect',...
                'new-correct','new-incorrect','NoResponse'};
